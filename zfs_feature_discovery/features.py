...
from typing import Any, AsyncContextManager, AsyncIterable, Mapping, cast
...
class FeatureManager(AsyncContextManager["FeatureManager"]):
    ...
    async def refresh_all_zpools(self) -> None:
        # make a copy to avoid any concurrency surprises
        zpools = list(self._zpools.values())

        await asyncio.gather(*(map(self.refresh_zpool, zpools)))
    ...
    async def refresh_all_zpool_datasets(self) -> None:
        # make a copy to avoid any concurrency surprises
        zpools = list(self._zpools.values())

        await asyncio.gather(*(map(self.refresh_zpool_datasets, zpools)))
